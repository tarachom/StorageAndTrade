...
{
	public partial class Form_ПсуванняТоварів_ТабличнаЧастина_Товари : UserControl
	{
		public Form_ПсуванняТоварів_ТабличнаЧастина_Товари()
		{
			InitializeComponent();
		...
		/// </summary>
		public Документи.ПсуванняТоварів_Objest ДокументОбєкт { get; set; }
		...
		private void ПсуванняТоварів_ТабличнаЧастина_Товари_Load(object sender, EventArgs e)
		{
		...
		public void LoadRecords()
		{
			...
			querySelect.Joins.Add(
				new Join(Довідники.Номенклатура_Const.TABLE, Документи.ПсуванняТоварів_Товари_TablePart.Номенклатура, querySelect.Table));

			//JOIN 2
			querySelect.FieldAndAlias.Add(
				new NameValue<string>(Довідники.ПакуванняОдиниціВиміру_Const.TABLE + "." + Довідники.ПакуванняОдиниціВиміру_Const.Назва, "pak_name"));
			querySelect.Joins.Add(
				new Join(Довідники.ПакуванняОдиниціВиміру_Const.TABLE, Документи.ПсуванняТоварів_Товари_TablePart.Пакування, querySelect.Table));
			...
			querySelect.Joins.Add(
				new Join(Довідники.ХарактеристикиНоменклатури_Const.TABLE, Документи.ПсуванняТоварів_Товари_TablePart.ХарактеристикаНоменклатури, querySelect.Table));

			//ORDER
			querySelect.Order.Add(Документи.ПсуванняТоварів_Товари_TablePart.НомерРядка, SelectOrder.ASC);
			...
			foreach (Документи.ПсуванняТоварів_Товари_TablePart.Record record in ДокументОбєкт.Товари_TablePart.Records)
			{
		...
		public void SaveRecords()
		{
			...
			foreach (Записи запис in RecordsBindingList)
			{
				Документи.ПсуванняТоварів_Товари_TablePart.Record record = new Документи.ПсуванняТоварів_Товари_TablePart.Record();
		...
		private class Записи
		{
			...
			public static void ОтриматиЦіну(Записи запис, Документи.ПсуванняТоварів_Objest ДокументОбєкт)
			{
				if (запис.Номенклатура.IsEmpty())
					return;
...
WHERE
    ЦіниНоменклатури.{РегістриВідомостей.ЦіниНоменклатури_Const.ВидЦіни} = '{ДокументОбєкт.ВидЦіни.UnigueID}' AND
