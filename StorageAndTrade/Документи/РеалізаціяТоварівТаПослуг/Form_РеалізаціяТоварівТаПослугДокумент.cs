...
namespace StorageAndTrade
{
    public partial class Form_РеалізаціяТоварівТаПослугДокумент : Form
    {
			...
			if (IsNew.HasValue)
			{
				...
				else
				{
					if (реалізаціяТоварівТаПослуг_Objest.Read(new UnigueID(Uid)))
					{
						...
						ComboBoxNameValue<Перелічення.ГосподарськіОперації>.SelectItem(comboBox_ГосподарськаОперація, реалізаціяТоварівТаПослуг_Objest.ГосподарськаОперація);
		...
		{
			if (реалізаціяТоварівТаПослуг_Objest.IsSave)
				РухДокументівПоРегістрах.PrintRecords(реалізаціяТоварівТаПослуг_Objest.GetDocumentPointer());
		}
