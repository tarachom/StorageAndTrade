    ...
    public partial class Form_РеалізаціяТоварівТаПослугДокумент : Form
    {
        ...
        private void Form_РеалізаціяТоварівТаПослугДокумент_Load(object sender, EventArgs e)
        {
			...
			if (IsNew.HasValue)
			{
				...
				if (IsNew.Value)
				{
					...
					directoryControl_Контрагент.DirectoryPointerItem = Константи.ЗначенняЗаЗамовчуванням.ОсновнийПокупець_Const.GetNewDirectoryPointer();
				...
				else
				{
					if (реалізаціяТоварівТаПослуг_Objest.Read(new UnigueID(Uid)))
					{
						...
						dateTimePicker_ДатаДок.Value = dateTimePicker_ЧасДок.Value = реалізаціяТоварівТаПослуг_Objest.ДатаДок;
        ...
        private void SaveDoc(bool spendDoc, bool closeForm)
		{
			if (IsNew.HasValue)
			{
				...
				реалізаціяТоварівТаПослуг_Objest.ДатаДок = ФункціїДляДокументів.ОбєднатиДатуТаЧас(dateTimePicker_ДатаДок.Value, dateTimePicker_ЧасДок.Value);
