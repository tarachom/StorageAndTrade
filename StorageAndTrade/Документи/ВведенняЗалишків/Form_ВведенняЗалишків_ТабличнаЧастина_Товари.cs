...
{
    public partial class Form_ВведенняЗалишків_ТабличнаЧастина_Товари : UserControl
    {
		...
		public void LoadRecords()
		{
			...
			querySelect.Joins.Add(
				new Join(Довідники.Номенклатура_Const.TABLE, Документи.ВведенняЗалишків_Товари_TablePart.Номенклатура, querySelect.Table));

			//JOIN 2
			querySelect.FieldAndAlias.Add(
				new NameValue<string>(Довідники.ПакуванняОдиниціВиміру_Const.TABLE + "." + Довідники.ПакуванняОдиниціВиміру_Const.Назва, "pak_name"));
			querySelect.Joins.Add(
				new Join(Довідники.ПакуванняОдиниціВиміру_Const.TABLE, Документи.ВведенняЗалишків_Товари_TablePart.Пакування, querySelect.Table));
			...
			querySelect.Joins.Add(
				new Join(Довідники.ХарактеристикиНоменклатури_Const.TABLE, Документи.ВведенняЗалишків_Товари_TablePart.ХарактеристикаНоменклатури, querySelect.Table));

			//JOIN 4
			querySelect.FieldAndAlias.Add(
				new NameValue<string>(Довідники.СеріїНоменклатури_Const.TABLE + "." + Довідники.СеріїНоменклатури_Const.Номер, "seria_number"));
			querySelect.Joins.Add(
				new Join(Довідники.СеріїНоменклатури_Const.TABLE, Документи.ВведенняЗалишків_Товари_TablePart.Серія, querySelect.Table));

			//ORDER
			querySelect.Order.Add(Документи.ВведенняЗалишків_Товари_TablePart.НомерРядка, SelectOrder.ASC);
			...
			foreach (Документи.ВведенняЗалишків_Товари_TablePart.Record record in ДокументОбєкт.Товари_TablePart.Records)
			{
		...
		public void SaveRecords()
        {
			...
			foreach (Записи запис in RecordsBindingList)
            {
				...
				Документи.ВведенняЗалишків_Товари_TablePart.Record record = new Документи.ВведенняЗалишків_Товари_TablePart.Record();
