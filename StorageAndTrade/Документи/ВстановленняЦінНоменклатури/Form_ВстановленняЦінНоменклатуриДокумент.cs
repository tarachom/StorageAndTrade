...
namespace StorageAndTrade
{
    public partial class Form_ВстановленняЦінНоменклатуриДокумент : Form
    {
        ...
        private void Form_ВстановленняЦінНоменклатуриДокумент_Load(object sender, EventArgs e)
        {
			...
			if (IsNew.HasValue)
			{
				...
				if (IsNew.Value)
				{
					...
					directoryControl_ВидЦіни.DirectoryPointerItem = Константи.ЗначенняЗаЗамовчуванням.ОсновнийВидЦіни_Const.GetNewDirectoryPointer();
				}
				else
				{
		...
		{
			if (встановленняЦінНоменклатури_Objest.IsSave)
				Звіти.РухДокументівПоРегістрах.PrintRecords(встановленняЦінНоменклатури_Objest.GetDocumentPointer());
		}
