		...
		private void реалізаціяТоварівТаПослугtoolStripMenuItem_Click(object sender, EventArgs e)
        {
			if (dataGridViewRecords.SelectedRows.Count > 0)
			{
				...
				if (form_РеалізаціяТоварівТаПослугЖурнал == null)
				{
					...
					((Form_РеалізаціяТоварівТаПослугЖурнал)form_РеалізаціяТоварівТаПослугЖурнал).SelectPointerItem = реалізаціяТоварівТаПослуг_Новий.GetDocumentPointer();
		...
		private void замовленняПостачальникуtoolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (dataGridViewRecords.SelectedRows.Count > 0)
			{
				...
				if (form_ЗамовленняПостачальникуЖурнал == null)
				{
					...
					((Form_ЗамовленняПостачальникуЖурнал)form_ЗамовленняПостачальникуЖурнал).SelectPointerItem = замовленняПостачальнику_Новий.GetDocumentPointer();
        ...
        private void поступленняТоварівТаПослугToolStripMenuItem_Click(object sender, EventArgs e)
        {
			if (dataGridViewRecords.SelectedRows.Count > 0)
			{
				...
				if (form_ПоступленняТоварівТаПослугЖурнал == null)
				{
					...
					((Form_ПоступленняТоварівТаПослугЖурнал)form_ПоступленняТоварівТаПослугЖурнал).SelectPointerItem = поступленняТоварівТаПослуг_Новий.GetDocumentPointer();
