...
GROUP BY 
    period_day, Організація, ДокументПоступлення, Номенклатура, ХарактеристикаНоменклатури
HAVING 
   SUM(CASE WHEN Рег_ПартіїТоварів.income = true THEN 
        Рег_ПартіїТоварів.{ПартіїТоварів_Const.Кількість} ELSE 
...
GROUP BY 
    Період, Організація, ДокументПоступлення, Номенклатура, ХарактеристикаНоменклатури
HAVING 
