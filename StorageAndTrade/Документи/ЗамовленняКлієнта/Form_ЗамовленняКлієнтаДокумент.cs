...
namespace StorageAndTrade
{
    public partial class Form_ЗамовленняКлієнтаДокумент : Form
    {
				...
				else
				{
					if (замовленняКлієнта_Objest.Read(new UnigueID(Uid)))
					{
						...
						foreach (NameValue<Перелічення.ГосподарськіОперації> операція in comboBox_ГосподарськаОперація.Items)
							if (операція.Value == замовленняКлієнта_Objest.ГосподарськаОперація)
							{
		...
		{
			if (замовленняКлієнта_Objest.IsSave)
				Звіти.РухДокументівПоРегістрах.PrintRecords(замовленняКлієнта_Objest.GetDocumentPointer());
		}
