...
(
	SELECT
	   ...
	   SUM(register.Кількість) OVER(ORDER BY Довідник_ПартіяТоварівКомпозит.{ПартіяТоварівКомпозит_Const.Дата}) >= @Кількість AS ДостатняКількість
	FROM
	   register
...
";
		...
		public static List<Dictionary<string, object>> ОтриматиРезервТоваруПоДокументу(
			Номенклатура_Pointer Номенклатура, ХарактеристикиНоменклатури_Pointer ХарактеристикаНоменклатури, Склади_Pointer Склад, UnigueID Owner)
		{
			string query = $@"
WITH register AS
(
...
";
		...
		public static List<Dictionary<string, object>> ОтриматиЗамовленняТоваруПоДокументу(
			Номенклатура_Pointer Номенклатура, ХарактеристикиНоменклатури_Pointer ХарактеристикаНоменклатури, Склади_Pointer Склад, UnigueID Owner)
		{
			string query = $@"
WITH register AS
(
	...
	WHERE
        ЗамовленняКлієнтів.owner = '{Owner}'
		...
		public static bool Spend(ЗамовленняКлієнта_Objest ДокументОбєкт)
		{
			...
			foreach (ЗамовленняКлієнта_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
			{
				if (ТовариРядок.Номенклатура.IsEmpty())
				{
					...
					СпільніФункції.ДокументНеПроводиться(СписокПомилок);
				}
			...
			foreach (ЗамовленняКлієнта_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
			{
				...
				if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
				{
					...
					record.ХарактеристикаНоменклатури = ТовариРядок.ХарактеристикаНоменклатури;
					record.Склад = !ТовариРядок.Склад.IsEmpty() ? ТовариРядок.Склад : ДокументОбєкт.Склад;
					record.ВРезервіПідЗамовлення = ТовариРядок.Кількість;
				}
			}
		...
		public static bool Spend(РеалізаціяТоварівТаПослуг_Objest ДокументОбєкт)
		{
			...
			foreach (РеалізаціяТоварівТаПослуг_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
			{
				...
				if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
				{
					...
					List<Dictionary<string, object>> Залишок = СпільніФункції.ОтриматиЗалишкиТоваруНаСкладі(
						ТовариРядок.Номенклатура, ТовариРядок.ХарактеристикаНоменклатури, ТовариРядок.Серія, 
						!ТовариРядок.Склад.IsEmpty() ? ТовариРядок.Склад : ДокументОбєкт.Склад,
						ДокументОбєкт.UnigueID, ДокументОбєкт.ДатаДок);

					ЗалишокНоменклатури.Add(ТовариРядок.НомерРядка, Залишок.Count > 0 ? (decimal)Залишок[0]["ВНаявності"] : 0);
					...
					if (ЗалишокНоменклатури[ТовариРядок.НомерРядка] > 0)
					{
						...
						//
						// Якщо є залишок тоді потрібно перевірити резерв
						//
						...
						if (!ТовариРядок.РахунокФактура.IsEmpty())
						{
							...
							List<Dictionary<string, object>> РезервРахункуФактури = СпільніФункції.ОтриматиРезервТоваруПоДокументу(
								ТовариРядок.Номенклатура, ТовариРядок.ХарактеристикаНоменклатури,
								!ТовариРядок.Склад.IsEmpty() ? ТовариРядок.Склад : ДокументОбєкт.Склад,
								ТовариРядок.РахунокФактура.UnigueID);

							if (РезервРахункуФактури.Count > 0)
							{
								decimal ВРезервіЗіСкладу = (decimal)РезервРахункуФактури[0]["ВРезервіЗіСкладу"];
								РезервРахунку.Add(ТовариРядок.НомерРядка, ВРезервіЗіСкладу >= ТовариРядок.Кількість ? ТовариРядок.Кількість : ВРезервіЗіСкладу);
							}
							else
							{
								СписокПомилок.Add($"Рядок {ТовариРядок.НомерРядка}. " +
						...
						{
							//
							// Якщо документу фактури немає, рахуємо всі резерви
							//
							...
							РезервНоменклатури.Add(ТовариРядок.НомерРядка, Резерв.Count > 0 ? (decimal)Резерв[0]["ВРезервіЗіСкладу"] : 0);
						...
						{
							List<Dictionary<string, object>> Замовлення = СпільніФункції.ОтриматиЗамовленняТоваруПоДокументу(
								ТовариРядок.Номенклатура, ТовариРядок.ХарактеристикаНоменклатури,
								!ТовариРядок.Склад.IsEmpty() ? ТовариРядок.Склад : ДокументОбєкт.Склад,
								ТовариРядок.ЗамовленняКлієнта.UnigueID);

							if (Замовлення.Count > 0)
							{
								decimal Замовлено = (decimal)Замовлення[0]["Замовлено"];
								ЗамволенняКлієнта.Add(ТовариРядок.НомерРядка, Замовлено >= ТовариРядок.Кількість ? ТовариРядок.Кількість : Замовлено);
							}
							else
							{
								СписокПомилок.Add($"Рядок {ТовариРядок.НомерРядка}. " +
									$"Вказаний документ Замовлення клієнта, не замовляв товар {СписокНоменклатури[ТовариРядок.НомерРядка].Назва}");
								СпільніФункції.ДокументНеПроводиться(СписокПомилок);
							}
						}
					}
			...
			foreach (РеалізаціяТоварівТаПослуг_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
			{
				if (!ТовариРядок.ЗамовленняКлієнта.IsEmpty())
				{
					...
					record.ХарактеристикаНоменклатури = ТовариРядок.ХарактеристикаНоменклатури;
					record.Склад = !ТовариРядок.Склад.IsEmpty() ? ТовариРядок.Склад : ДокументОбєкт.Склад;
					record.Замовлено = ЗамволенняКлієнта[ТовариРядок.НомерРядка];
					record.Сума = ТовариРядок.Сума;
				}
			}
			...
			foreach (РеалізаціяТоварівТаПослуг_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
			{
				if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
				{
					...
					if (!ТовариРядок.РахунокФактура.IsEmpty())
						record.ВРезервіЗіСкладу = РезервРахунку[ТовариРядок.НомерРядка];

					if (!ТовариРядок.ЗамовленняКлієнта.IsEmpty())
						record.ВРезервіПідЗамовлення = ЗамволенняКлієнта[ТовариРядок.НомерРядка];
				}
			}
...
";
			...
			paramQuery.Add("Організація", ДокументОбєкт.Організація.UnigueID.UGuid); //Звернути увагу чи потрібно це
		...
		public static bool Spend(ПоверненняТоварівВідКлієнта_Objest ДокументОбєкт)
		{
			...
			foreach (ПоверненняТоварівВідКлієнта_Товари_TablePart.Record Товари_Record in ДокументОбєкт.Товари_TablePart.Records)
			{
				...
				List<Dictionary<string, object>> listNameRow = ОтриматиПартіїТоваруЗДокументуРеалізації(ДокументОбєкт, Товари_Record);
				...
				if (КількістьЯкуПотрібноПовернути > 0)
				{
					Console.WriteLine($"Невистачило повернути {КількістьЯкуПотрібноПовернути} товарів");
				}
			}
...
";
	...
	{
		//private static List<Dictionary<string, object>> ОтриматиСписокПартій(ПереміщенняТоварів_Objest ДокументОбєкт,
		//	ПереміщенняТоварів_Товари_TablePart.Record ТовариРядок)
		//{
			//
...
//        AND ПартіїТоварів.{ПартіїТоварів_Const.Склад} = '{ДокументОбєкт.СкладВідправник.UnigueID}'
...
//		}
		...
		private static List<Dictionary<string, object>> ОтриматиЗалишкиТоварівНаСкладах(ПереміщенняТоварів_Objest ДокументОбєкт,
			ПереміщенняТоварів_Товари_TablePart.Record ТовариРядок)
		{
			string query = $@"
WITH register AS
...
";
			...
			Console.WriteLine(ДокументОбєкт.ДатаДок);
			paramQuery.Add("period_end", ДокументОбєкт.ДатаДок);
			paramQuery.Add("Склад", ДокументОбєкт.СкладВідправник.UnigueID.UGuid);
		...
		public static bool Spend(ПереміщенняТоварів_Objest ДокументОбєкт)
		{
			...
			foreach (ПереміщенняТоварів_Товари_TablePart.Record Товари_Record in ДокументОбєкт.Товари_TablePart.Records)
			{
				...
				{
					List<Dictionary<string, object>> listNameRow = СпільніФункції.ОтриматиСписокНаявнихПартій(
						ДокументОбєкт.Організація, Товари_Record.Номенклатура, Товари_Record.ХарактеристикаНоменклатури,
						Товари_Record.Серія, ДокументОбєкт.СкладВідправник, ДокументОбєкт.UnigueID, ДокументОбєкт.ДатаДок, Товари_Record.Кількість);
...
//		private static List<Dictionary<string, object>> ОтриматиСписокПартій(ВнутрішнєСпоживанняТоварів_Objest ДокументОбєкт,
//			ВнутрішнєСпоживанняТоварів_Товари_TablePart.Record ТовариРядок)
//		{
//			//
//			//Вибірка даних з регістру, так як Віртуальні залишки розраховуються із запізненням
//			//і в основному віртуальні залишки потрібні для звітів.
...
//		AND ПартіїТоварів.{ПартіїТоварів_Const.ХарактеристикаНоменклатури} = '{ТовариРядок.ХарактеристикаНоменклатури.UnigueID}'
//		AND ПартіїТоварів.{ПартіїТоварів_Const.Серія} = '{ТовариРядок.Серія.UnigueID}'
//        AND ПартіїТоварів.{ПартіїТоварів_Const.Склад} = '{ДокументОбєкт.Склад.UnigueID}'
//        AND ПартіїТоварів.owner != '{ДокументОбєкт.UnigueID}'
...
//";
		...
		private static List<Dictionary<string, object>> ОтриматиЗалишкиТоварівНаСкладах(ВнутрішнєСпоживанняТоварів_Objest ДокументОбєкт,
			ВнутрішнєСпоживанняТоварів_Товари_TablePart.Record ТовариРядок)
		{
			string query = $@"
WITH register AS
		...
		public static bool Spend(ВнутрішнєСпоживанняТоварів_Objest ДокументОбєкт)
		{
			...
			foreach (ВнутрішнєСпоживанняТоварів_Товари_TablePart.Record Товари_Record in ДокументОбєкт.Товари_TablePart.Records)
			{
				...
				if (СписокНоменклатури[Товари_Record.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
				{
					List<Dictionary<string, object>> listNameRow = СпільніФункції.ОтриматиСписокНаявнихПартій(
						ДокументОбєкт.Організація, Товари_Record.Номенклатура, Товари_Record.ХарактеристикаНоменклатури,
						Товари_Record.Серія, ДокументОбєкт.Склад, ДокументОбєкт.UnigueID, ДокументОбєкт.ДатаДок, Товари_Record.Кількість);
