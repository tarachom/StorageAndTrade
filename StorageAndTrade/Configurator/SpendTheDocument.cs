				...
				if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
				{
					...
					if (ЗалишокНоменклатури[ТовариРядок.НомерРядка] > 0)
					{
						...
						if (!ТовариРядок.РахунокФактура.IsEmpty())
						{
							...
							{
								РезервРахунку.Add(ТовариРядок.НомерРядка, (decimal)РезервРахункуФактури[0]["ВРезервіЗіСкладу"]);

								//if (РезервРахунку[ТовариРядок.НомерРядка] < ТовариРядок.Кількість)
								//{
								//	СписокПомилок.Add($"Рядок {ТовариРядок.НомерРядка}. " +
								//		$"Рахунок фактура зарезервував {РезервРахунку[ТовариРядок.НомерРядка]}, " +
								//		$"а потрібно {ТовариРядок.Кількість}");
								//	СпільніФункції.ДокументНеПроводиться(СписокПомилок);
								//}
							}
							else
