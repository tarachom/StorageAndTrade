    ...
    public partial class Form_КасиЕлемент : Form
    {
		...
		private void Form_КасиЕлемент_Load(object sender, EventArgs e)
        {
			...
			if (IsNew.HasValue)
			{
				...
				{
					this.Text += " - Новий";
					textBox_Код.Text = каси_Objest.Код = (++Константи.НумераціяДовідників.Каси_Const).ToString("D6");
				}
				else
				{
					if (каси_Objest.Read(new UnigueID(Uid)))
					{
						...
						textBox_Код.Text = каси_Objest.Код;
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
			if (IsNew.HasValue)
			{
				...
				try
				{
					...
					каси_Objest.Код = textBox_Код.Text;
				...
				if (OwnerForm != null)
				{
					OwnerForm.SelectPointerItem = каси_Objest.GetDirectoryPointer();
