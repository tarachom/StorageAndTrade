...
WHERE date_trunc('day', {ВіртуальніТаблиціРегістрів.РухКоштів_День_TablePart.Період}) = '{Період}';
...
SELECT 
    uuid_generate_v4(),
    date_trunc('day', Рег_РухКоштів.period) as period_month,
...
WHERE
    date_trunc('day', Рег_РухКоштів.period) = '{Період}'
GROUP BY 
...
WHERE date_trunc('month', {ВіртуальніТаблиціРегістрів.РухКоштів_Місяць_TablePart.Період}) = '{Місяць}';
...
SELECT 
    uuid_generate_v4(),
    date_trunc('month', РухКоштів_День.{ВіртуальніТаблиціРегістрів.РухКоштів_День_TablePart.Період}) as Період,
...
WHERE
    date_trunc('month', РухКоштів_День.{ВіртуальніТаблиціРегістрів.РухКоштів_День_TablePart.Період}) = '{Місяць}'
GROUP BY 
