...
(
	SELECT
	   ...
	   Кількість,
	   Собівартість,
	   SUM(Кількість) OVER(ORDER BY Документ_ПоступленняТоварівТаПослуг.{ПоступленняТоварівТаПослуг_Const.ДатаДок}) >= 
		  {ТовариРядок.Кількість} AS ДостатняКількість
	FROM
	   register
