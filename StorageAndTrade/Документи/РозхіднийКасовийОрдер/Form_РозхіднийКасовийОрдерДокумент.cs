...
namespace StorageAndTrade
{
    public partial class Form_РозхіднийКасовийОрдерДокумент : Form
    {
        ...
        private void Form_РозхіднийКасовийОрдерДокумент_Load(object sender, EventArgs e)
        {
			...
			if (IsNew.HasValue)
			{
				...
				if (IsNew.Value)
				{
					...
					directoryControl_БанківськийРахунок.DirectoryPointerItem = Константи.ЗначенняЗаЗамовчуванням.ОсновнийБанківськийРахунок_Const.GetNewDirectoryPointer();
		...
		{
			if (розхіднийКасовийОрдер_Objest.IsSave)
				Звіти.РухДокументівПоРегістрах.PrintRecords(розхіднийКасовийОрдер_Objest.GetDocumentPointer());
		}
