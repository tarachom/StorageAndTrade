...
GROUP BY 
    period_day, Організація, Документ, Номенклатура, ХарактеристикаНоменклатури, Серія
HAVING 
   SUM(CASE WHEN Рег_ПартіїТоварів.income = true THEN 
        Рег_ПартіїТоварів.{ПартіїТоварів_Const.Кількість} ELSE 
...
GROUP BY 
    Період, Організація, Документ, Номенклатура, ХарактеристикаНоменклатури, Серія
HAVING 
