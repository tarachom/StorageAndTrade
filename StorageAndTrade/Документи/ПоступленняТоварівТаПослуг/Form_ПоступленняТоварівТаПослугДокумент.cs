    ...
    public partial class Form_ПоступленняТоварівТаПослугДокумент : Form
    {
        ...
        private void SaveDoc(bool spendDoc, bool closeForm)
		{
			if (IsNew.HasValue)
			{
				...
				//поступленняТоварівТаПослуг_Objest.ПартіяТоварівКомпозит = StorageAndTrade.ФункціїДляДокументів.ОтриматиПартіюТоварівКомпозит(
				//	поступленняТоварівТаПослуг_Objest.ПартіяТоварівКомпозит, Перелічення.ТипДокументуПартіяТоварівКомпозит.ПоступленняТоварівТаПослуг,
				//	поступленняТоварівТаПослуг_Objest, null);
