...
namespace StorageAndTrade
{
    public partial class Form_ЗамовленняПостачальникуДокумент : Form
    {
        ...
        private void Form_ЗамовленняПостачальникуДокумент_Load(object sender, EventArgs e)
        {
			...
			if (IsNew.HasValue)
			{
				...
				if (IsNew.Value)
				{
					...
					directoryControl_Контрагент.DirectoryPointerItem = Константи.ЗначенняЗаЗамовчуванням.ОсновнийПостачальник_Const.GetNewDirectoryPointer();
					...
					directoryControl_Валюта.DirectoryPointerItem = Константи.ЗначенняЗаЗамовчуванням.ОсновнаВалюта_Const.GetNewDirectoryPointer();
					directoryControl_Склад.DirectoryPointerItem = Константи.ЗначенняЗаЗамовчуванням.ОснонийСклад_Const.GetNewDirectoryPointer();
					directoryControl_Каса.DirectoryPointerItem = Константи.ЗначенняЗаЗамовчуванням.ОсновнаКаса_Const.GetNewDirectoryPointer();
		...
		{
			if (замовленняПостачальнику_Objest.IsSave)
			{
				if (OwnerForm != null && !OwnerForm.IsDisposed)
				{
					OwnerForm.SelectPointerItem = замовленняПостачальнику_Objest.GetDocumentPointer();
				...
				{
					Form_ЗамовленняПостачальникуЖурнал form_Журнал = new Form_ЗамовленняПостачальникуЖурнал();
					form_Журнал.MdiParent = this.MdiParent;
					form_Журнал.SelectPointerItem = замовленняПостачальнику_Objest.GetDocumentPointer();
					form_Журнал.Show();
				}
			}
		}
		...
		{
			if (замовленняПостачальнику_Objest.IsSave)
				Звіти.РухДокументівПоРегістрах.PrintRecords(замовленняПостачальнику_Objest.GetDocumentPointer());
		}
