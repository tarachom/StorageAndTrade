...
{
    public partial class Form_ПереміщенняТоварів_ТабличнаЧастина_Товари : UserControl
    {
        public Form_ПереміщенняТоварів_ТабличнаЧастина_Товари()
        {
            InitializeComponent();
        }
		...
		/// </summary>
		public Документи.ПереміщенняТоварів_Objest ДокументОбєкт { get; set; }

        private void Переміщення_ТабличнаЧастина_Товари_Load(object sender, EventArgs e)
        {
		...
		public void LoadRecords()
		{
			...
			querySelect.Joins.Add(
				new Join(Довідники.Номенклатура_Const.TABLE, Документи.ПереміщенняТоварів_Товари_TablePart.Номенклатура, querySelect.Table));

			//JOIN 2
			querySelect.FieldAndAlias.Add(
				new NameValue<string>(Довідники.ПакуванняОдиниціВиміру_Const.TABLE + "." + Довідники.ПакуванняОдиниціВиміру_Const.Назва, "pak_name"));
			querySelect.Joins.Add(
				new Join(Довідники.ПакуванняОдиниціВиміру_Const.TABLE, Документи.ПереміщенняТоварів_Товари_TablePart.Пакування, querySelect.Table));
			...
			querySelect.Joins.Add(
				new Join(Довідники.ХарактеристикиНоменклатури_Const.TABLE, Документи.ПереміщенняТоварів_Товари_TablePart.ХарактеристикаНоменклатури, querySelect.Table));

			//JOIN 4
			querySelect.FieldAndAlias.Add(
				new NameValue<string>(Довідники.СеріїНоменклатури_Const.TABLE + "." + Довідники.СеріїНоменклатури_Const.Номер, "seria_number"));
			querySelect.Joins.Add(
				new Join(Довідники.СеріїНоменклатури_Const.TABLE, Документи.ПереміщенняТоварів_Товари_TablePart.Серія, querySelect.Table));

			//ORDER
			querySelect.Order.Add(Документи.ПереміщенняТоварів_Товари_TablePart.НомерРядка, SelectOrder.ASC);
			...
			foreach (Документи.ПереміщенняТоварів_Товари_TablePart.Record record in ДокументОбєкт.Товари_TablePart.Records)
			{
		...
		public void SaveRecords()
        {
            ...
            {				
				Документи.ПереміщенняТоварів_Товари_TablePart.Record record = new Документи.ПереміщенняТоварів_Товари_TablePart.Record();
