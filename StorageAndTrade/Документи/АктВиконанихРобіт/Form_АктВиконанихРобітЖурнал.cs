    ...
    public partial class Form_АктВиконанихРобітЖурнал : Form
    {
		...
		public void LoadRecords()
		{
			...
			if (SelectPointerItem == null && RecordsBindingList.Count > 0)
				SelectPointerItem = new Документи.АктВиконанихРобіт_Pointer(new UnigueID(RecordsBindingList[RecordsBindingList.Count - 1].ID));
		...
		private void dataGridViewRecords_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (dataGridViewRecords.SelectedRows.Count > 0)
			{
				...
				SelectPointerItem = new Документи.АктВиконанихРобіт_Pointer(new UnigueID(dataGridViewRecords.Rows[e.RowIndex].Cells["ID"].Value.ToString()));
			}
		}
