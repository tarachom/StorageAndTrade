...
namespace StorageAndTrade
{
    public partial class Form_ВнутрішнєСпоживанняТоварівДокумент : Form
    {
				...
				else
				{
					if (внутрішнєСпоживанняТоварів_Objest.Read(new UnigueID(Uid)))
					{
						...
						foreach (NameValue<Перелічення.ГосподарськіОперації> операція in comboBox_ГосподарськаОперація.Items)
							if (операція.Value == внутрішнєСпоживанняТоварів_Objest.ГосподарськаОперація)
							{
		...
		{
			if (внутрішнєСпоживанняТоварів_Objest.IsSave)
				Звіти.РухДокументівПоРегістрах.PrintRecords(внутрішнєСпоживанняТоварів_Objest.GetDocumentPointer());
		}
